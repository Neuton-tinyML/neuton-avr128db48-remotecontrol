 ...
 * @param[out] p_index              For Classification: Index of predicted target(class) with highest probability. For Regression this parameter is not used
 * @param[out] pp_outputs           Set pointer to the internal buffer with all model outputs.
 *                                  Model outputs contains predicted target value (for regression task) or probabilities 
 *                                  of each class (binary/multi classification)
 * 
 * @return neuton_i16_t       Number of the predicted outputs(classes/values) or the following errors:
